...
def build_sinusoidal_positional_encoding(max_len, d_model):
    ...
    position = build_position_index_column(max_len)      # (max_len, 1)
    div_term = compute_positional_div_term(d_model)       # (d_model // 2,)
...
def softmax_attention_weights(masked_scores):
    all_masked = torch.isinf(masked_scores).all(dim=-1, keepdim=True)
...
def assemble_multi_head_attention_forward(query, key, value, w_q, w_k, w_v, w_o, num_heads, mask=None):
    # Project each input stream with its own weight matrix (no bias provided in this signature)
    q = apply_linear_projection(query, w_q, None)
    k = apply_linear_projection(key, w_k, None)
    v = apply_linear_projection(value, w_v, None)

    # Split each projected tensor into per-head views
    ...
    # Merge heads back together and apply the output projection
...
def encoder_layer_self_attention_sublayer(x, w_q, w_k, w_v, w_o, gamma, beta, num_heads, src_mask):
    attn_out = assemble_multi_head_attention_forward(x, x, x, w_q, w_k, w_v, w_o, num_heads, mask=src_mask)
